...
pub mod pallet {
	...
	const HTTP_REMOTE_API: &str = "https://api.coincap.io/v2/assets/polkadot";
	...
	struct DotPriceInfo {
		// Specify our own deserializing function to convert JSON string to vector of bytes
		#[serde(
			deserialize_with = "de_string_to_bytes",
			rename(serialize = "priceUsd", deserialize = "priceUsd")
		)]
		price_usd: Vec<u8>,
	}
	#[derive(Debug, Deserialize, Encode, Decode, Default)]
	...
	impl<T: Config> Pallet<T> {
		#[pallet::weight(10000)]
		pub fn submit_price_signed(origin: OriginFor<T>, pu: u64, pm: Permill) -> DispatchResult {
			let who = ensure_signed(origin)?;
			log::info!("submit_price_signed: ({}, {:?}, {:?})", pu, pm, who);
			Self::append_or_replace_price(pu, pm);

			Self::deposit_event(Event::NewPrice(Some(who), pu, pm));
			Ok(())
		}
	...
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_price(pu: u64, pm: Permill) {
			Prices::<T>::mutate(|prices| {
				if prices.len() == NUM_VEC_LEN {
					let _ = prices.pop_front();
				}
				prices.push_back((pu, pm));
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			let mut pu: u64 = 0;
			let mut pm: Permill = Permill::from_percent(0);

			match Self::fetch_n_api_parse() {
				Ok(dp_data) => {
					log::info!("dp_data: {:?}", dp_data);
					let price = dp_data.data.price_usd;
					log::info!("dp_price: {:?}", price);
			...
			// 回写到链上
			// 这里使用 签名交易 来回写到链上
			// 签名交易 应该属于传统的，标准的，符合常理的上链方式
			Self::offchain_signed_tx_price(pu, pm);
		...
		fn fetch_n_api_parse() -> Result<DotPriceData, Error<T>> {
			let resp_bytes = Self::fetch_from_remote_api().map_err(|e| {
				log::error!("fetch_from_remote_api error: {:?}", e);
				<Error<T>>::HttpFetchingError
			})?;
			...
			let resp_str =
				str::from_utf8(&resp_bytes).map_err(|_| <Error<T>>::HttpFetchingError)?;
			// Print out our fetched JSON string
			log::info!("api resp_str {}", resp_str);
		...
		fn fetch_from_remote_api() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", HTTP_REMOTE_API);

			// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
			let request = rt_offchain::http::Request::get(HTTP_REMOTE_API);
		...
		fn offchain_signed_tx_price(pu: u64, pm: Permill) -> Result<(), Error<T>> {
			...
			let result = signer.send_signed_transaction(|_acct|
				...
				Call::submit_price_signed(pu, pm));
